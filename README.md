# Routine-API
매 주마다 정해진 일정에 자신이 해야할 일을 등록하고, 해당 수행 여부에 대한 내용을 기록하여 관리할 수 있도록 도와주는 Routine-API
...
## 요구사항 분석
✅ Routine 기능: 로그인한 유저만 접근 가능하며 유저 식별은 토큰을 사용합니다.
...
|-----|----------|---------------------------|----------------------------|----------------------------|---|
| v1  | 유저       | POST /account/signup/     |  회원가입    | 회원가입                       | ✅   |
| -   | -        | POST /account/login/      | 로그인       | jwt를 이용한 로그인               | ✅   |
| -   | Routine  | POST /routines/           | 생성        | Routine 생성                 | ✅   |
| -   | -        | GET /routines/?date=      |조회        | Routine 전체 조회(날짜별)         | ✅   |
| -   | -        | GET /routines/:id/        |조회        | Routine 상세 조회        | ✅   |
| -   | -        | PUT /routines/:id/        |수정        | Routine 수정                 | ✅   | 
| -   | -        | DELETE /routines/:id/     |삭제        | Routine 삭제(soft-delete)    | ✅   |
| -   | -        | GET /routines/:id/result/ |결과 조회     | (현재 요일에 해야할)Routine의 결과 조회 | ✅   |
| -   | -        | PUT /routines/:id/check   |결과 기록     | (현재 요일에 해야할)Routine의 결과 기록 | ✅   |
